 ...
 * Tx and Rx timers
 */
sx1272_timer_t TxTimeoutTimer;
sx1272_timer_t RxTimeoutTimer;
sx1272_timer_t RxTimeoutSyncWord;

/*
 * Radio driver functions implementation
 */

void SX1272InitIRQ(uint8_t dio0, uint8_t dio1, uint8_t dio2, uint8_t dio3, uint8_t dio4, uint8_t dio5)
{
	if(dio0 != 0 || dio0 != PIN_UNCONNECTED) SX1272.dio0 = dio0;
	if(dio1 != 0 || dio1 != PIN_UNCONNECTED) SX1272.dio1 = dio1;
	if(dio2 != 0 || dio2 != PIN_UNCONNECTED) SX1272.dio2 = dio2;
	if(dio3 != 0 || dio3 != PIN_UNCONNECTED) SX1272.dio3 = dio3;
	if(dio4 != 0 || dio4 != PIN_UNCONNECTED) SX1272.dio4 = dio4;
	if(dio5 != 0 || dio5 != PIN_UNCONNECTED) SX1272.dio5 = dio5;
    SX1272IoIrqInit( DioIrq );
}

void SX1272InitDIO(uint8_t nss, uint8_t rst, uint8_t ctrl, uint8_t vdd)
{
	if(nss != 0 || nss != PIN_UNCONNECTED) SX1272.nss  = nss;
	if(rst != 0 || rst != PIN_UNCONNECTED) SX1272.rst  = rst;
	if(vdd != 0 || vdd != PIN_UNCONNECTED) SX1272.vdd  = vdd;
	if(ctrl!= 0 || ctrl!= PIN_UNCONNECTED) SX1272.ctrl = ctrl;
	SX1272IoInit();
}
 
void SX1272Init( SPIClass* spi, RadioEvents_t *events )
{
    uint8_t i;
...
void SX1272Handle(void)
{
	TimerProcess( &TxTimeoutTimer );
	TimerProcess( &RxTimeoutTimer );
	TimerProcess( &RxTimeoutSyncWord );
}
...
static uint8_t GetFskBandwidthRegValue( uint32_t bandwidth )
{
    ...
    while( 1 ) Serial.println("Internal Error (GetFskBandwidthRegValue)! Device halted!");
}
...
void SX1272WriteBuffer( uint16_t addr, uint8_t *buffer, uint8_t size )
{
	...
	SX1272.spi->transfer(buf, size+1);
    digitalWrite(SX1272.nss, HIGH);
}
...
void SX1272ReadBuffer( uint16_t addr, uint8_t *buffer, uint8_t size )
{
	...
	uint8_t bufIn[256];
	bufOut[0] = addr & 0x7F;
	SX1272.spi->transfer(bufOut, bufIn, size+1);
	memcpy(buffer, &bufIn[1], size);
    digitalWrite(SX1272.nss, HIGH);
}
    ...
    switch( SX1272.Settings.State )
    {
        case RF_RX_RUNNING:
            ...
            case MODEM_FSK:
                ...
                if( SX1272.dio4 == PIN_UNCONNECTED )
                {
...
uint8_t SpiInOut( SPIClass* spi, uint8_t outData )
{
